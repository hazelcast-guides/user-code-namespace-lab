...
public class AuthorizationPipeline {
    ...
    public static Pipeline createPipeline(String kafkaBootstrapServers, String inputTopic, String outputTopic){
         ...
         * a result.
         */
        StreamStage<Transaction> postBigTxn = transactions.map( txn -> {
           if (txn.getAmount() > 5000)
               txn.setStatus(Transaction.Status.DECLINED_BIG_TXN);
         ...
         */
        StreamStage<Transaction> postLocked = postBigTxn.groupingKey( txn -> txn.getCardNumber())
                .<Card, Transaction>mapUsingIMap(Names.CARD_MAP_NAME,
                        (txn, card) -> {
                            if (txn.getStatus() == Transaction.Status.NEW){
                                if (card.getLocked()){
                                    txn.setStatus(Transaction.Status.DECLINED_LOCKED);
                                }
                            }
                            return txn;
                        })
         ...
         */
        StreamStage<Transaction> postAuthorized = postLocked.filter( t -> true);
